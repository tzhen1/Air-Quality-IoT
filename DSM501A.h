#include<string.h>
#define PM1PIN 12//DSM501A input D6 on ESP8266 in GPIO12
#define PM25PIN 14
byte buff[2];
...
float calculateConcentrationSM(long lowpulseInMicroSeconds, long durationinSeconds){
...
float ratio(long lowpulseInMicroSeconds){
  float ratioValue = (lowpulseInMicroSeconds/1000000.0)/30.0*100.0;
  return ratioValue;
}
...
void loopDSM501A()
{
  ...
  if ((endtime-starttime) > sampletime_ms) //Only after 30s has passed we calcualte the ratio
  {
    ...
    */
    float conPM1 = calculateConcentrationSM(lowpulseoccupancyPM1,30);
